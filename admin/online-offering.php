...
if (isset($_POST['delete'])) {
  $id=$_POST['id'];
  $name=$_POST['name'];
  $sql_delete = "DELETE FROM online_offering WHERE online_offering_id='$id';";
...
if(isset($_POST['isConfirmed?'])) {
  ...
  if (mysqli_query($connect, 'UPDATE online_offering SET offering_isconfirmed ='.$value.' WHERE online_offering_id =' .$_POST['id'])) {
    mysqli_close($connect);
    header('Location: ' . $_SERVER['PHP_SELF']);
  }
}
...
if (isset($_POST['update'])) {
 ...
 $sql_update = "UPDATE online_offering SET course_id='$course_id', trainer_id='$trainer_id', offering_startdate='$offering_startdate', offering_enddate='$offering_enddate', offering_starttime='$offering_starttime', offering_endtime='$offering_endtime', offering_timezone='$offering_timezone', offering_price='$offering_price', offering_isprivate='$offering_isprivate' WHERE online_offering_id='$id';";
...
if (isset($_POST['submit'])) {
  ...
  $insert = 'INSERT INTO online_offering (`course_id`, `trainer_id`, `offering_startdate`, `offering_enddate`, `offering_starttime`, `offering_endtime`, `offering_timezone`, `offering_price`, `offering_isprivate`) VALUES ("'  . $_POST['courseID'] . '","' . $_POST['trainerID'] . '","' . $_POST['stDate'] . '","' . $_POST['endDate'] . '","' . $_POST['stTime'] .':00'. '","' . $_POST['endTime'].'","' . $_POST['timezone']. '","' . $_POST['price'] . '","' . $_POST['priOrNot'] .'")';
...
?>
      <h2 class="p-2" style="color:gray;border-bottom:1px solid black"><i class="fa fa-globe"></i> Online Offerings</h2>
  <table class="table table-striped">
    ...
    <tbody>
      ...
      while ($row = mysqli_fetch_array($result)) {
        ...
        $row4 = mysqli_fetch_assoc(mysqli_query($connect, 'SELECT COUNT(*) FROM online_enrollment where online_offering_id = ' . $row['online_offering_id']));
        echo '<tr>
              ...
              <button type="button" class="btn btn-primary btn-sm edit" title="Edit row" data-name="' . $row2['course_name'] . '" data-courseid="' . $row2['course_id'] . '" data-inperson="' . $b . '" data-trainer="' . $row3['staff_id'] . '" data-startdate="' . $row['offering_startdate'] . '" data-enddate="' . $row['offering_enddate'] . '" data-starttime="' . $row['offering_starttime'] . '" data-endtime="' . $row['offering_endtime'] . '" data-timezone="' . $row['offering_timezone'] . '" data-price="' . $row['offering_price'] . '" data-id="' . $row['online_offering_id'] . '"><i class="fa fa-edit"></i> Edit</button>
              <button type="button" class="btn btn-danger btn-sm delete" del-id="'  . $row['online_offering_id'] . '" del-name="' . $row2['course_name']  . '" title="Delete row"><i class="fa fa-trash"></i> Delete</button>
              
              <div>
                  <form action="" method="POST">
                    <input type="hidden" name="id" value='.$row['online_offering_id'].'>';
              ...
              echo '</div>
              <div class="mt-2"><a class="btn btn-outline-primary btn-sm" href="'.'online-enrollment.php'.'?OoID='.$row['online_offering_id'].'">See enrollments</a></div>
              </td>
              </tr>';
        $index++;
      }
      ...
      <div class="modal-body">
        ...
        <input type="hidden" name="id" id="online_id" value="">
          ...
          <div class="mb-3">
            ...
            <select class="form-select" name="trainerID" id="trainerID">
